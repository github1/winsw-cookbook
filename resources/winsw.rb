...
property :winsw_bin_url, String, default: 'https://github.com/winsw/winsw/releases/download/v2.12.0/WinSW-x64.exe'
...
action :install do
  ...
  configuration_sets.each do |key, config|
    ...
    if ::File.exist?(new_resource.winsw_bin_url)
      winsw_download_path = new_resource.winsw_bin_url
    else
    ...
    link config[:service_exec] do
      notifies :run, "execute[#{key} stop re-configured service]", :before
      to winsw_download_path
      link_type :symbolic
    end
    ...
    file config[:service_descriptor_xml_path] do
      ...
      notifies :run, "execute[#{key} restart re-configured service]", :immediately if new_resource.enabled && !reinstall_required
    end
